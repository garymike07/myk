...
class PortfolioApp {
    ...
    render() {
        ...
        root.innerHTML = `
            ${this.renderNavbar()}
            ${this.renderHero()}
            ${this.renderGovernmentActivities()}
            ${this.renderAbout()}
            ${this.renderSkills()}
            ...
            <section class="hero-bg min-h-screen flex items-center justify-center pt-20">
                <div class="container mx-auto px-6 text-center">
                    <div class="animate-fadeInUp">
                        <img src="${this.data.about.profilePicture}" alt="Profile Picture" 
                             class="profile-picture w-32 h-32 rounded-full mx-auto mb-8 object-cover">
                        <h1 class="text-5xl md:text-6xl font-bold mb-4">
                            <span class="text-gradient">${this.data.about.name}</span>
                        </h1>
                        <h2 class="text-2xl md:text-3xl mb-6 text-[var(--text-secondary)]">
    ...
    loadDashboardContent() {
        ...
        switch (this.currentDashboardTab) {
            case 'about':
                content.innerHTML = this.renderAboutDashboard();
                break;
            case 'skills':
                content.innerHTML = this.renderSkillsDashboard();
                break;
            case 'experience':
                content.innerHTML = this.renderExperienceDashboard();
                break;
            case 'projects':
                content.innerHTML = this.renderProjectsDashboard();
                break;
            case 'blogs':
                content.innerHTML = this.renderBlogsDashboard();
                break;
            case 'social':
                content.innerHTML = this.renderSocialDashboard();
                break;
        }
    }

    renderAboutDashboard() {
        return `
            <div class="space-y-6">
                <div class="form-group">
                    <label class="form-label">Name</label>
                    <input type="text" id="about-name" class="form-input" value="${this.data.about.name}">
                </div>
                <div class="form-group">
                    <label class="form-label">Title</label>
                    <input type="text" id="about-title" class="form-input" value="${this.data.about.title}">
                </div>
                <div class="form-group">
                    <label class="form-label">Description</label>
                    <textarea id="about-description" class="form-input form-textarea">${this.data.about.description}</textarea>
                </div>
                <div class="form-group">
    ...
    renderSkillsDashboard() {
        return `
            <div class="space-y-6">
                <div class="form-group">
                    <label class="form-label">Technical Skills (one per line)</label>
                    <textarea id="technical-skills" class="form-input form-textarea">${this.data.skills.technical.join('\n')}</textarea>
                </div>
                <div class="form-group">
                    <label class="form-label">Soft Skills (one per line)</label>
                    <textarea id="soft-skills" class="form-input form-textarea">${this.data.skills.soft.join('\n')}</textarea>
                </div>
                <div class="form-group">
                    <label class="form-label">Languages (one per line)</label>
                    <textarea id="languages" class="form-input form-textarea">${this.data.skills.languages.join('\n')}</textarea>
                </div>
            </div>
        `;
    }
    ...
    renderExperienceDashboard() {
        return `
            <div class="space-y-6">
                ${this.data.experience.map((exp, index) => `
                    <div class="card">
                        <h3 class="text-lg font-semibold mb-4 text-[var(--primary-color)]">Experience ${index + 1}</h3>
                        <div class="space-y-4">
                            <div class="form-group">
                                <label class="form-label">Role</label>
                                <input type="text" id="exp-role-${index}" class="form-input" value="${exp.role}">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Date</label>
                                <input type="text" id="exp-date-${index}" class="form-input" value="${exp.date}">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Description</label>
                                <textarea id="exp-desc-${index}" class="form-input form-textarea">${exp.description}</textarea>
                            </div>
                        </div>
                    </div>
                `).join('')}
    ...
    renderProjectsDashboard() {
        return `
            <div class="space-y-6">
                ${this.data.projects.map((project, index) => `
                    <div class="card">
                        <h3 class="text-lg font-semibold mb-4 text-[var(--primary-color)]">Project ${index + 1}</h3>
                        <div class="space-y-4">
                            <div class="form-group">
                                <label class="form-label">Title</label>
                                <input type="text" id="project-title-${index}" class="form-input" value="${project.title}">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Description</label>
                                <textarea id="project-desc-${index}" class="form-input form-textarea">${project.description}</textarea>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Thumbnail</label>
                                <div class="file-upload">
                                    <input type="file" id="project-thumbnail-${index}" accept="image/*">
                                    <p>Click to upload project thumbnail</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    renderBlogsDashboard() {
        return `
            <div class="space-y-6">
                ${this.data.blogs.map((blog, index) => `
                    <div class="card">
                        <h3 class="text-lg font-semibold mb-4 text-[var(--primary-color)]">Blog ${index + 1}</h3>
                        <div class="space-y-4">
                            <div class="form-group">
                                <label class="form-label">Title</label>
                                <input type="text" id="blog-title-${index}" class="form-input" value="${blog.title}">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Excerpt</label>
                                <textarea id="blog-excerpt-${index}" class="form-input form-textarea">${blog.excerpt}</textarea>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Thumbnail</label>
                                <div class="file-upload">
                                    <input type="file" id="blog-thumbnail-${index}" accept="image/*">
                                    <p>Click to upload blog thumbnail</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    renderSocialDashboard() {
        return `
            <div class="space-y-6">
                ${this.data.socialLinks.map((link, index) => `
                    <div class="card">
                        <h3 class="text-lg font-semibold mb-4 text-[var(--primary-color)]">${link.platform}</h3>
                        <div class="form-group">
                            <label class="form-label">URL</label>
                            <input type="url" id="social-url-${index}" class="form-input" value="${link.url}">
                        </div>
                    </div>
                `).join('')}
            </div>
    ...
    async saveChanges() {
        ...
        switch (this.currentDashboardTab) {
            case 'about':
                updatedData.about.name = document.getElementById('about-name').value;
                updatedData.about.title = document.getElementById('about-title').value;
                updatedData.about.description = document.getElementById('about-description').value;
                break;
            case 'skills':
                updatedData.skills.technical = document.getElementById('technical-skills').value.split('\n').filter(s => s.trim());
                updatedData.skills.soft = document.getElementById('soft-skills').value.split('\n').filter(s => s.trim());
                updatedData.skills.languages = document.getElementById('languages').value.split('\n').filter(s => s.trim());
                break;
            case 'experience':
                this.data.experience.forEach((exp, index) => {
                    updatedData.experience[index] = {
                        role: document.getElementById(`exp-role-${index}`).value,
                        date: document.getElementById(`exp-date-${index}`).value,
            ...
            case 'social':
                this.data.socialLinks.forEach((link, index) => {
                    updatedData.socialLinks[index] = {
                        ...link,
                        url: document.getElementById(`social-url-${index}`).value
                    };
                });
                break;
        }
        ...
        // Handle file uploads
        const fileInputs = {
            profilePicture: document.getElementById('profile-picture-upload'),
        };

        for (let i = 0; i < this.data.projects.length; i++) {
            fileInputs[`projectThumbnail${i}`] = document.getElementById(`project-thumbnail-${i}`);
        }
        for (let i = 0; i < this.data.blogs.length; i++) {
            fileInputs[`blogThumbnail${i}`] = document.getElementById(`blog-thumbnail-${i}`);
        }
            ...
            if (input && input.files && input.files[0]) {
                ...
                reader.onload = (e) => {
                    ...
                    if (key === 'profilePicture') {
                        updatedData.about.profilePicture = newPath;
                    } else if (key.startsWith('projectThumbnail')) {
                        const index = parseInt(key.replace('projectThumbnail', ''));
                        updatedData.projects[index].thumbnail = newPath;
                    } else if (key.startsWith('blogThumbnail')) {
                        const index = parseInt(key.replace('blogThumbnail', ''));
                        updatedData.blogs[index].thumbnail = newPath;
                    }
                    // In a real scenario, you would upload the file to a server here
                    // For GitHub Pages, you'd need a separate workflow (e.g., GitHub Actions)
                    // For now, we'll just update the path in data.json
                };
        ...
        this.downloadDataJSON();
        alert('Changes saved successfully! Download the updated data.json file. For image/video changes, you will need to manually replace the files in the assets folder and redeploy.');
    }
    ...
    renderGovernmentActivities() {
        const images = [
            'assets/1000019787.jpg',
            'assets/1000019776.jpg',
            'assets/1000019784.jpg',
            'assets/1000019783.jpg',
            'assets/1000019777.webp',
            'assets/1000019778.webp',
            'assets/1000019789.jpg'
        ];
        ...
        return `
            <section id="government-activities" class="py-20 bg-[#1a1a1a]">
                <div class="container mx-auto px-6">
                    <div class="text-center mb-16">
                        <h2 class="text-4xl font-bold mb-4 text-gradient">Previous Government Activities Participated</h2>
                        <div class="w-20 h-1 bg-gradient-to-r from-[var(--primary-color)] to-[var(--secondary-color)] mx-auto"></div>
                    </div>
                    <div class="government-activities-container">
                        <div class="government-activities-scroll">
                            ${duplicatedImages.map(src => `
                                <img src="${src}" alt="Government Activity" class="government-activity-image">
                            `).join('')}
                        </div>
                    </div>
                </div>
